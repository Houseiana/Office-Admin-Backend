...
{
    [HttpGet]
    public async Task<ActionResult<ApiResponse<PagedResponse<EmployeeListDto>>>> GetEmployees(
        ...
        [FromQuery] int pageSize = 20)
    {
        ...
        return Ok(ApiResponse<PagedResponse<EmployeeListDto>>.Ok(result));
    }

    [HttpGet("{id:guid}")]
    public async Task<ActionResult<ApiResponse<EmployeeDetailDto>>> GetEmployee(Guid id)
    {
    ...
    [HttpPost]
    public async Task<ActionResult<ApiResponse<EmployeeDetailDto>>> CreateEmployee(CreateEmployeeDto dto)
    {
        var employee = await employeeService.CreateEmployeeAsync(dto);
        return CreatedAtAction(nameof(GetEmployee), new { id = employee.Id },
            ApiResponse<EmployeeDetailDto>.Ok(employee, "Employee created"));
    }

    [HttpPut("{id:guid}")]
    public async Task<ActionResult<ApiResponse<EmployeeDetailDto>>> UpdateEmployee(Guid id, UpdateEmployeeDto dto)
    {
        var employee = await employeeService.UpdateEmployeeAsync(id, dto);
        if (employee == null)
            return NotFound(ApiResponse<EmployeeDetailDto>.Fail("Employee not found"));

        return Ok(ApiResponse<EmployeeDetailDto>.Ok(employee, "Employee updated"));
    }
